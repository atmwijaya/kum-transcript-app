...
const DaftarAnggota = () => {
  ...
  // Syarat minimum untuk jenjang Bhakti (harus sama dengan backend)
  const [syaratMinimal, setSyaratMinimal] = useState({
    pendidikan: 79,
    kegiatan: 89,
    latihan: 43
  });

  // Fetch data anggota Madya dari API
  const fetchAnggotaMadya = async () => {
    try {
      setIsLoading(true);
      setError(null);

      // Endpoint untuk mendapatkan semua anggota dengan filter jenjang
      ...
      // Filter hanya anggota dengan jenjang MADYA (jika API belum filter)
      const anggotaMadya = data.filter(member => 
        member && member.jenjang && member.jenjang.toLowerCase() === 'madya'
      );
      
      setAnggota(anggotaMadya);
      
      // Setelah mendapatkan data anggota, fetch nilai kumulatif mereka
  ...
  // Fetch data nilai kumulatif untuk semua anggota sekaligus
  const fetchNilaiKumulatifBatch = async (anggotaList) => {
    try {
      // Gunakan endpoint baru yang sudah dibuat
      const response = await fetch(`${API_BASE_URL}/api/nilai-kumulatif?jenjang=madya&limit=100`);
      ...
      console.log('Data nilai kumulatif:', result);
      
      if (result.success) {
        // Konversi data dari API menjadi format yang mudah digunakan
        const nilaiMap = {};
        
        // Jika ada data nilai dari API
        ...
        // Untuk anggota yang tidak memiliki data nilai kumulatif, buat entri kosong
      ...
      // Jika error, buat data kosong untuk semua anggota
      const emptyNilai = {};
      anggotaList.forEach(member => {
  ...
  // Fetch data nilai kumulatif untuk satu anggota
  const fetchNilaiKumulatifSingle = async (anggotaId) => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/nilai-kumulatif/anggota/${anggotaId}`);
      ...
      console.error('Error fetching single nilai:', err);
      return null;
    }
  };
  ...
  const getFilteredAndSortedData = () => {
    ...
    if (sortConfig.key) {
      filtered.sort((a, b) => {
        ...
        // Sorting berdasarkan nilai kumulatif
  ...
  // Fungsi untuk menambah/mengupdate nilai
  const handleTambahNilai = async (memberId) => {
    // Tampilkan modal atau form untuk input nilai
    const jenisNilai = prompt('Pilih jenis nilai:\n1. Pendidikan\n2. Kegiatan\n3. Latihan\n\nMasukkan angka 1-3:');
    const nilaiStr = prompt('Masukkan nilai (0-100):');
    
    if (!jenisNilai || !nilaiStr) return;
    ...
    let kategori;
    switch (jenisNilai) {
      case '1': kategori = 'pendidikan'; break;
      case '2': kategori = 'kegiatan'; break;
      case '3': kategori = 'latihan'; break;
      default: 
        alert('Jenis nilai tidak valid');
        return;
    }
    ...
    try {
      ...
      if (nilaiData && nilaiData._id) {
        ...
        response = await fetch(`${API_BASE_URL}/api/nilai-kumulatif/${nilaiData._id}/${kategori}`, {
          method: 'PATCH',
          headers: {
        ...
        // Buat nilai baru
        // Pertama cek apakah data nilai sudah ada
        const existingData = await fetchNilaiKumulatifSingle(memberId);
        
        if (existingData && existingData._id) {
          // Update jika sudah ada
          response = await fetch(`${API_BASE_URL}/api/nilai-kumulatif/${existingData._id}/${kategori}`, {
            method: 'PATCH',
            headers: {
              'Content-Type': 'application/json',
            },
          ...
          response = await fetch(`${API_BASE_URL}/api/nilai-kumulatif`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
      ...
      if (result.success) {
        ...
        // Refresh data nilai untuk anggota ini
        const updatedNilai = await fetchNilaiKumulatifSingle(memberId);
        if (updatedNilai) {
          setNilaiKumulatif(prev => ({
            ...prev,
            [memberId]: {
              _id: updatedNilai._id,
              pendidikan: updatedNilai.pendidikan || 0,
              kegiatan: updatedNilai.kegiatan || 0,
              latihan: updatedNilai.latihan || 0,
              isLantikBhakti: updatedNilai.is_lantik_bhakti || false,
              tanggal_dilantik: updatedNilai.tanggal_dilantik,
              catatan: updatedNilai.catatan || '',
              lastUpdated: updatedNilai.updated_at,
              nama: updatedNilai.anggota_id?.nama || '',
              angkatan: updatedNilai.anggota_id?.angkatan || '',
              jenjang: updatedNilai.anggota_id?.jenjang || '',
              fakultas: updatedNilai.anggota_id?.fakultas || '',
              jurusan: updatedNilai.anggota_id?.jurusan || '',
              nim: updatedNilai.anggota_id?.nim || ''
            }
          }));
        }
      } else {
  ...
  // Fungsi untuk melantik menjadi Bhakti
  const handleLantikBhakti = async (memberId, memberName) => {
    if (!window.confirm(`Apakah Anda yakin ingin melantik ${memberName} menjadi Bhakti?`)) {
      return;
    }
    ...
    const memenuhiSyarat = cekMemenuhiBhakti(memberId);
    if (!memenuhiSyarat) {
      alert('Anggota belum memenuhi syarat minimal untuk dilantik Bhakti');
      return;
    }
    
    try {
      const response = await fetch(`${API_BASE_URL}/api/nilai-kumulatif/${nilaiData._id}/lantik-bhakti`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (!response.ok) {
        throw new Error('Gagal melantik anggota');
      }
      ...
      if (result.success) {
        ...
        alert(`Gagal melantik: ${result.message}`);
      }
      
    } catch (error) {
      console.error('Error lantik bhakti:', error);
      alert('Terjadi kesalahan saat melantik anggota');
    }
  };

  // Fungsi untuk menghapus data nilai kumulatif (hanya untuk yang sudah Bhakti)
  const handleHapusNilai = async (memberId, memberName) => {
    if (!window.confirm(`Apakah Anda yakin ingin menghapus data nilai kumulatif untuk ${memberName}?\n\nIni hanya diperbolehkan untuk anggota yang sudah dilantik Bhakti.`)) {
      return;
    }
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/nilai-kumulatif/${nilaiData._id}`, {
        method: 'DELETE',
      });
  ...
  // Format nilai dengan warna dan cek syarat minimal (DIMODIFIKASI: menghilangkan text minimum di bawah)
  const formatNilai = (nilai, jenis, memberId) => {
    ...
    return (
      <span className={`inline-flex items-center justify-center px-3 py-1.5 rounded-lg text-sm font-medium ${colorClass} min-w-[60px]`}>
        {nilai}
      </span>
    );
  };
  ...
  // Skeleton Loading Component (sama seperti sebelumnya)
  const SkeletonRow = () => (
    ...
    <div className="p-4 md:p-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
          ...
          <div className="bg-gradient-to-r from-red-500 to-red-600 text-white px-4 py-2 rounded-lg self-start md:self-auto">
            <div className="text-center">
              <div className="text-lg md:text-xl font-bold">{anggota.length}</div>
        ...
        <div className="flex flex-wrap items-center gap-2 text-sm text-gray-600">
          ...
          {selectedAngkatan !== 'semua' && (
            <span className="inline-flex items-center bg-blue-100 text-blue-700 text-xs px-3 py-1 rounded-full">
              ...
              <button onClick={() => setSelectedAngkatan('semua')} className="ml-2 hover:text-blue-900">×</button>
            </span>
          )}
          {searchTerm && (
          ...
          {sortConfig.key && (
            <span className="inline-flex items-center bg-purple-100 text-purple-700 text-xs px-3 py-1 rounded-full">
              ...
              <button onClick={() => setSortConfig({ key: null, direction: 'ascending' })} className="ml-2 hover:text-purple-900">×</button>
            </span>
          )}
        </div>
      </div>
      ...
      {isLoading && (
        <>
          ...
          {/* Mobile Skeleton */}
          <div className="md:hidden bg-white rounded-xl shadow overflow-hidden mb-6">
      ...
      {!isLoading && !error && (
        <>
          {/* Tabel Anggota - MODIFIED: Menghilangkan kolom total */}
          <div className="bg-white rounded-xl shadow overflow-hidden mb-6">
            ...
            {/* Syarat Minimal di Header Tabel (Desktop) - MODIFIED: Menghilangkan kolom untuk total */}
            <div className="hidden md:block px-6 py-3 bg-gray-50 border-b">
              <div className="grid grid-cols-7 gap-4">
                <div className="col-span-2 text-sm font-medium text-gray-700">Syarat Minimal Bhakti:</div>
                <div className="text-center">
                  <div className="text-xs text-gray-500">Pendidikan</div>
                  <div className="text-sm font-bold text-blue-600">{syaratMinimal.pendidikan}</div>
                </div>
                <div className="text-center">
                  <div className="text-xs text-gray-500">Kegiatan</div>
                  <div className="text-sm font-bold text-green-600">{syaratMinimal.kegiatan}</div>
                </div>
                <div className="text-center">
                  <div className="text-xs text-gray-500">Latihan</div>
                  <div className="text-sm font-bold text-yellow-600">{syaratMinimal.latihan}</div>
                </div>
                <div className="col-span-2"></div>
              </div>
            </div>

            {/* Desktop View - MODIFIED: Menghilangkan kolom total dan sort untuk total */}
                        ...
                        <tr key={member._id} className={`hover:bg-gray-50 transition-colors ${
                          ...
                          <td className="px-6 py-4">
                            <div>
                              ...
                              <div className="text-xs text-gray-500">
                                {member.fakultas && member.jurusan ? `${member.fakultas} - ${member.jurusan}` : '-'}
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex justify-center">
                              {formatNilai(nilai.pendidikan || 0, 'pendidikan', member._id)}
                            </div>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex justify-center">
                              {formatNilai(nilai.kegiatan || 0, 'kegiatan', member._id)}
                            </div>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex justify-center">
                              {formatNilai(nilai.latihan || 0, 'latihan', member._id)}
                            </div>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex items-center gap-2">
                              <button
                                onClick={() => handleTambahNilai(member._id)}
                                className="p-2 text-blue-600 hover:bg-blue-100 rounded-lg transition-colors"
                                title="Tambah/Ubah Nilai"
                              >
                                <PlusCircleIcon className="h-5 w-5" />
                              </button>
                              
                              {memenuhiSyarat && !isLantikBhakti && (
                                <button
                                  onClick={() => handleLantikBhakti(member._id, member.nama)}
                                  className="p-2 text-green-600 hover:bg-green-100 rounded-lg transition-colors"
                                  title="Lantik menjadi Bhakti"
                                >
            ...
            {/* Mobile View - MODIFIED: Menghilangkan total dan mengubah grid dari 4 kolom menjadi 3 kolom */}
            <div className="md:hidden">
              {/* Syarat Minimal di Header (Mobile) - MODIFIED: Menghilangkan total */}
              <div className="px-4 py-3 bg-gray-50 border-b">
                <div className="text-sm font-medium text-gray-700 mb-2">Syarat Minimal Bhakti:</div>
              ...
              {/* Daftar Anggota Mobile - MODIFIED: Menghilangkan total */}
              <div className="divide-y divide-gray-200">
                {filteredData.length > 0 ? (
                  filteredData.map((member, index) => {
                    const nilai = nilaiKumulatif[member._id] || {};
                    ...
                    return (
                      <div key={member._id} className={`p-4 hover:bg-gray-50 transition-colors ${
                        ...
                        {/* MODIFIED: Mengubah dari grid-cols-4 menjadi grid-cols-3 */}
                        <div className="grid grid-cols-3 gap-3 mt-4">
                          <div>
                            <div className="text-xs text-gray-500 mb-1 text-center">Pendidikan</div>
                            <div className="flex justify-center">
                              {formatNilai(nilai.pendidikan || 0, 'pendidikan', member._id)}
                            </div>
                          </div>
                          <div>
                            <div className="text-xs text-gray-500 mb-1 text-center">Kegiatan</div>
                            <div className="flex justify-center">
                              {formatNilai(nilai.kegiatan || 0, 'kegiatan', member._id)}
                            </div>
                          </div>
                          <div>
                            <div className="text-xs text-gray-500 mb-1 text-center">Latihan</div>
                            <div className="flex justify-center">
                              {formatNilai(nilai.latihan || 0, 'latihan', member._id)}
                            </div>
                          </div>
                        ...
                        <div className="mt-4 pt-4 border-t border-gray-100 flex justify-between items-center">
                          <div className="text-xs text-gray-500">
                            {isLantikBhakti ? '✓ Sudah dilantik Bhakti' : 
                             memenuhiSyarat ? '✓ Memenuhi syarat Bhakti' : 'Belum memenuhi syarat'}
                          </div>
          ...
          {/* Statistik Section */}
          <div className="mb-6">
            <h3 className="text-lg font-semibold text-gray-800 mb-4">Statistik Nilai Kumulatif</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="bg-white rounded-xl shadow p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm text-gray-500">Sudah Memenuhi Syarat</p>
                    <p className="text-2xl font-bold text-gray-800">
                      {Object.values(nilaiKumulatif).filter(nilai => 
                        cekMemenuhiBhakti(Object.keys(nilaiKumulatif).find(key => nilaiKumulatif[key] === nilai))
                      ).length}
                    </p>
                  </div>
              ...
              <div className="bg-white rounded-xl shadow p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm text-gray-500">Sudah Dilantik Bhakti</p>
                    <p className="text-2xl font-bold text-gray-800">
                      {Object.values(nilaiKumulatif).filter(nilai => nilai.isLantikBhakti).length}
                    </p>
                  </div>
          ...
          <div className="mt-6 p-4 bg-red-50 rounded-lg border border-red-200">
            <div className="flex items-start">
              ...
              <div className="ml-3">
                <h3 className="text-sm font-medium text-red-800">Informasi Sistem Kenaikan Jenjang</h3>
                <div className="mt-2 text-sm text-red-700 space-y-2">
                  <p><strong>Kriteria Kenaikan ke Bhakti:</strong></p>
                  <ul className="list-disc pl-4 space-y-1">
                    <li>Anggota harus mencapai <strong>nilai minimal di semua kategori</strong></li>
                  ...
                  <p><strong>Indikator Warna:</strong></p>
                  <ul className="list-disc pl-4 space-y-1">
                    <li><span className="text-green-600 font-medium">Hijau:</span> Sudah dilantik Bhakti</li>
                    <li><span className="text-blue-600 font-medium">Biru:</span> Memenuhi syarat minimal</li>
                    <li><span className="text-red-600 font-medium">Merah:</span> Belum memenuhi syarat</li>
                  </ul>
                  
                  <p className="text-xs text-red-600/70 mt-3">
                    Note: Gunakan tombol (+) untuk menambah/ubah nilai, tombol (✓) untuk melantik anggota yang memenuhi syarat, 
